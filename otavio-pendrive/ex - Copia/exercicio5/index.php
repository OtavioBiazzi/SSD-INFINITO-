<!--
Data: 10/03/2026
Autor: Otávio Giovanelli Biazzi e Pedro Henrique Miranda
Objetivo:

Exercício 5 - Somatório de Fatoriais
Leia 5 números inteiros e mostre a soma dos fatoriais desses números.
-->

<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Somatório de Fatoriais</title>
</head>
<body>
    <h1>Soma dos Fatoriais</h1>

    <form method="post" action="calcula.php">
        Digite o primeiro número:
        <input type="number" name="n1" min="0" required><br><br>

        Digite o segundo número:
        <input type="number" name="n2" min="0" required><br><br>

        Digite o terceiro número:
        <input type="number" name="n3" min="0" required><br><br>

        Digite o quarto número:
        <input type="number" name="n4" min="0" required><br><br>

        Digite o quinto número:
        <input type="number" name="n5" min="0" required><br><br>

        <button type="submit">Calcular</button>
    </form>
</body>
</html>